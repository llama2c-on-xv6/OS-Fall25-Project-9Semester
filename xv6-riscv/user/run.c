...
#ifndef RD_TIME_FREQ
#define RD_TIME_FREQ 1000000ULL  // assumed 1MHz for QEMU / xv6riscv
#endif
...
void read_checkpoint(char* checkpoint_unused, Config* config, TransformerWeights* weights,
                     int* fd, float** data, ssize_t* file_size) {
    (void)checkpoint_unused; // kept for compatibility with callers

    // 1) fetch model weights over the network (returns malloc'd char* or 0)
    int fetched_size = 0;
    ...
    // 3) copy the Config header from the buffer
    memcpy(config, buf, sizeof(Config));
    int shared_weights = config->vocab_size > 0 ? 1 : 0;
    config->vocab_size = abs(config->vocab_size);

    // 4) pointer to weights region (immediately after Config)
    size_t weights_bytes = (size_t)fetched_size - sizeof(Config);
    char *weights_byte_ptr = buf + sizeof(Config);

    // 5) ensure float alignment; memory_map_weights expects float*
    if (((uintptr_t)weights_byte_ptr % sizeof(float)) != 0) {
        ...
        memcpy(aligned_weights, weights_byte_ptr, weights_bytes);

        // map the weights from aligned buffer
        memory_map_weights(weights, config, aligned_weights, shared_weights);

        // record the aligned buffer as the transformer's data to free later
        *data = aligned_weights;
        *file_size = (ssize_t)weights_bytes;
        *fd = -1;

        // free the original fetched buffer (we copied data out)
        free(buf);
    } else {
        // aligned: point directly into fetched buffer
        float *weights_ptr = (float*)weights_byte_ptr;
        memory_map_weights(weights, config, weights_ptr, shared_weights);

        // keep the fetched buffer so callers can free it when done
        *data = (float*)buf;
        *file_size = (ssize_t)fetched_size;
        *fd = -1;
    }
}
...
void free_transformer(Transformer* t) {
    // free the network-allocated buffer (or aligned buffer)
    if (t->data) {
        free((void*)t->data);
        t->data = NULL;
    }
    if (t->fd != -1) { close(t->fd); t->fd = -1; }
...
void matmul(float* xout, float* x, float* w, int n, int d) {
    ...
    int i;
    // #pragma omp parallel for private(i) -                                pragma removed 
    for (i = 0; i < d; i++) {
        float val = 0.0f;
        for (int j = 0; j < n; j++) {
            val += w[i * n + j] * x[j];
...
float* forward(Transformer* transformer, int token, int pos) {
    ...
    for(unsigned long long l = 0; l < p->n_layers; l++) {
        ...
        int h;
    //    #pragma omp parallel for private(h)             -   pragma romoved 
...
void build_tokenizer(Tokenizer* t, char* tokenizer_path_unused, int vocab_size) {
    (void)tokenizer_path_unused; // path ignored — we fetch over network
    ...
    // fetch tokenizer binary via network
    int tok_size = 0;
    ...
    // parse tokenizer buffer: [int max_token_length][for i: float score][int len][bytes...]]
    char *p = tok_buf;
    ...
    // we've copied all useful data; free tok_buf to avoid holding extra memory
    free(tok_buf);
}
...
// ----------------------------------------------------------------------------
// utilities: time (rdtime-based for xv6)

long time_in_ms(void) {
    // rdtime returns raw ticks (uint64). Convert to milliseconds.
    // We cast through double to avoid integer truncation for large numbers.
    uint64 t = rdtime(); // user-level syscall wrapper expected
    double ms = ((double)t * 1000.0) / (double)RD_TIME_FREQ;
    return (long)ms;
}
...
void generate(Transformer *transformer, Tokenizer *tokenizer, Sampler *sampler, char *prompt, int steps) {
    ...
    // start the main loop
    uint64 start = 0;  // rdtime ticks (uint64)
    int next;          // will store the next token in the sequence
    int token = prompt_tokens[0]; // kick off with the first token in the prompt
    int pos = 0;       // position in the sequence (number of produced tokens)
    while (pos < steps) {
        ...
        // init the rdtime timer here because the first iteration can be slower
        if (start == 0) { start = rdtime(); }
    }
    printf("\n");
    ...
    if (pos > 1) {
        ...
        else ticks = 0; // should not happen with monotonic rdtime, but be safe
